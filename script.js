    ...
    "achievements": [
        {
            ...
            "image": "images/john_johnson.jpeg"
        },
        {
            "score": 19,
            "date": "2026-04-06",
        ...
        },
        {
            ...
            "image": "images/annie_tedesco.jpeg"
        },
        {
            "score": 36,
            "date": "2026-03-11",
        ...
        },
        {
            ...
            "image": "images/andrew_mackenzie.jpeg"
        },
        {
            "score": 80,
            "date": "2025-09-12",
...
class CoupaScorigami {
    ...
    setupTooltip(cell, achievement) {
        ...
        cell.addEventListener('mouseenter', (e) => {
            ...
            hoverTimeout = setTimeout(() => {
                ...
                // Highlight the corresponding chart dot
                this.highlightChartDotForDate(achievement.date, true);
            }, 100);
        });
        ...
        cell.addEventListener('mouseleave', () => {
            ...
            // Remove highlight from chart dot
            this.highlightChartDotForDate(achievement.date, false);
        });
    ...
    highlightChartDotForDate(dateStr, shouldHighlight) {
        const svg = document.getElementById('progress-chart');
        if (!svg) return;
        
        // Find the chart dot with matching date
        const chartDots = svg.querySelectorAll('.chart-dot');
        chartDots.forEach(dot => {
            if (dot.dataset.dateStr === dateStr) {
                if (shouldHighlight) {
    ...
    showTooltip(event, achievement) {
        ...
        // Show all fields for grid cell tooltip
        tooltipImage.style.display = 'block';
        tooltipName.style.display = 'block';
        tooltipDate.style.display = 'block';
        tooltipScore.style.display = 'block';
    ...
    createProgressChart() {
        const svg = document.getElementById('progress-chart');
        if (!svg || this.achievements.length === 0) return;
        ...
        sortedAchievements.forEach(achievement => {
            ...
            if (!dataPointsByDate.has(dateStr)) {
                dataPointsByDate.set(dateStr, {
                ...
                // Update count and add score for same date
                const existingPoint = dataPointsByDate.get(dateStr);
                existingPoint.count = cumulativeCount;
                existingPoint.scores.push(achievement.score);
            }
        });
        ...
        const yScale = (count) => {
            const ratio = count / 100; // Fixed to 100 max scores
        ...
        // Draw dots for each achievement (excluding the current date point)
        dataPoints.slice(0, -1).forEach(point => {
            ...
            circle.dataset.scores = JSON.stringify(point.scores);

            // Add hover event to highlight grid squares and show date
            circle.addEventListener('mouseenter', (e) => {
                // Highlight the circle
                circle.classList.add('highlighted');
                
                // Highlight all corresponding grid cells for this date
                point.scores.forEach(score => {
                    const gridCell = this.grid.querySelector(`[data-score="${score}"]`);
                    if (gridCell) {
                        gridCell.classList.add('chart-highlighted');
                    }
                });

                // Show tooltip with just the date
                this.showDateTooltip(e, point.dateStr);
            });
            ...
            circle.addEventListener('mouseleave', () => {
                ...
                point.scores.forEach(score => {
                    const gridCell = this.grid.querySelector(`[data-score="${score}"]`);
                    if (gridCell) {
                        gridCell.classList.remove('chart-highlighted');
                    }
                });
        ...
        // Add axis labels
        // Y-axis label (count) - fixed to 100
    ...
    showDateTooltip(event, dateStr) {
        ...
        // Set tooltip content - just the date
        tooltipDate.textContent = this.formatDate(dateStr);
